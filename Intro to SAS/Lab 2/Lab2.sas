...
/*Zadanie 3-4*/
LIBNAME covid '/folders/myfolders/sasuser/covid';
LIBNAME covid list;
PROC IMPORT datafile='/folders/myfolders/sasuser/covid/worldometer_data.csv'
...
dbms=csv
outfile='/folders/myfolders/sasuser/cnt.csv';
...
data NEW_CARS(keep=Make Model Type);
	set SASHELP.CARS(keep=Make Model Type Cylinders Horsepower 
					WHERE=(Cylinders = 6 AND Horsepower >= 220));
run;
