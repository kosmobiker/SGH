...
Stopa wzrotu już jest podana w zbiorze danych za pomocą `BatchGetSymbols`, ale też obliczył ją samodzielnie
```{r, echo=FALSE}
res <- df %>% 
        group_by(ticker) %>% 
...
```{r, echo=FALSE}
fd <- fitdistr(ts_df$QCOM, "t", start = list(m=mean(ts_df$QCOM),s=sd(ts_df$QCOM), df=3), lower=c(-1, 0.001,1))
pander(fd)
```